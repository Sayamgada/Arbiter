...
export default function Home() {
  ...
  const [paymentStatus, setPaymentStatus] = useState<
    "idle" | "creating" | "ready" | "processing" | "authorized" | "failed"
  >("idle");
  ...
  async function handlePaymentSuccess(response: {
    ...
    try {
      ...
      console.log("Arbiter payment verified:", result);
    } catch (err) {
      setPaymentStatus("failed");
  ...
  function openRazorpayCheckout() {
    if (!paymentOrder) {
      setPaymentError("Payment order has not been created.");
      return;
    }
    ...
    const razorpay = new window.Razorpay({
      ...
      name: "Arbiter",
      description: "Trust-aware commerce transaction",
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6">
              ...
              <div className="mt-5 space-y-4">
                ...
                <Signal
                  label="Intent"
                  value={context.buyer.intent_confidence}
                />

                <Signal
                  label="History"
                  value={context.buyer.history_score}
                />

                <Signal
                  label="Behavior"
                  value={context.buyer.behavior_score}
                />
              ...
              <div className="mt-8 border-t border-zinc-800 pt-6">
                ...
                {negotiation.status === "accepted" && (
                  <div className="mt-8 flex flex-col items-center border-t border-zinc-800 pt-8">
                      ...
                      paymentStatus !== "authorized" && (
                        <div className="mt-6 w-full max-w-3xl rounded-xl border border-zinc-800 bg-black p-5">
                          ...
                          <button
                            ...
                            className="mt-6 w-full rounded-xl bg-white px-6 py-3 text-sm font-semibold text-black transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-50"
                          >
                            {paymentStatus === "processing"
                              ? "PAYMENT PROCESSING..."
                              : "PAY WITH RAZORPAY"}
                    ...
                    {/* Payment authorized */}
                    {paymentStatus === "authorized" && (
                      <div className="mt-6 w-full max-w-3xl rounded-2xl border border-emerald-900/60 bg-emerald-950/20 p-6">
                        <div className="flex items-center gap-4">
                          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-400 font-semibold text-black">
                            ✓
                          </div>
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm text-zinc-300">
                              Razorpay payment signature was
                              verified successfully by Arbiter.
                            </p>
                          </div>
                        </div>
